...
interface DeleteCarBrandDialogProps {
  isOpen: boolean
  onClose: () => void
  carBrand: ICarBrand | null
  onCarBrandDeleted: () => void
}

export function DeleteCarBrandDialog({ isOpen, onClose, carBrand, onCarBrandDeleted }: DeleteCarBrandDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsLoading(true)
    try {
      const result = await CarBrandService.deleteCarBrand(carBrand.id)

      if (result && result.errorCode === ServiceErrorCode.success) {
        ErrorService.successMessage("Marque supprimée", "La marque a été supprimée avec succès")
        onCarBrandDeleted()
        onClose()
      } else if (result?.errorCode === ServiceErrorCode.notFound) {
        ErrorService.errorMessage("Marque non trouvée", "Cette marque n'existe plus")
      } else {
        ErrorService.errorMessage("Erreur", "Impossible de supprimer la marque")
      }
    } catch (error) {
      ErrorService.errorMessage("Erreur", "Une erreur est survenue lors de la suppression de la marque")
    } finally {
      setIsLoading(false)
    }
  }
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-700">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-white">Supprimer la marque</h2>
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center space-x-3 p-4 bg-gray-700/50 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">ID: {carBrand.id}</p>
                </div>
              </div>
              ...
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-gray-400">
                  Cette action ne peut pas être annulée et supprimera aussi tous les modèles associés.
                </p>
              </div>
