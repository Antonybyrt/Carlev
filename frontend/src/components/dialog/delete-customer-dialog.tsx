...
import { X, User, Trash2, Loader2, AlertTriangle } from "lucide-react"
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result && result.errorCode === ServiceErrorCode.success) {
        ErrorService.successMessage("Client supprimé", "Le client a été supprimé avec succès")
        onCustomerDeleted()
        onClose()
      } else if (result?.errorCode === ServiceErrorCode.notFound) {
        ErrorService.errorMessage("Client non trouvé", "Ce client n'existe plus")
      } else {
        ErrorService.errorMessage("Erreur", "Impossible de supprimer le client")
      }
    } catch (error) {
      ErrorService.errorMessage("Erreur", "Une erreur est survenue lors de la suppression du client")
    } finally {
      setIsLoading(false)
    }
  }
          ...
          >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-700">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h2 className="text-xl font-semibold text-white">Supprimer le client</h2>
